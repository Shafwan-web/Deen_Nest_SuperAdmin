...
export default function Plans() {
  ...
  return (
    <div className="font-primary">
      ...
      {/* Pricing Calculator--------------------------------------------------------------------- */}
      <div className="bg-[#FFFFFF] rounded-xl text-[#1E293B] p-6 w-[450px] h-auto mt-5">
          ...
          <div className="border border-[#f2f2f2] mt-4 p-4 rounded-lg">
            ...
            <div className="border border-[#F2F2F2] rounded-sm mt-4 p-4">
              <div className="flex flex-col justify-between gap-4">
                ...
                <span className="text-sm text-[#6B7280] font-semibold pl-5">
                  Additional storage space for
                  <br /> documents and media files
                </span>
                <div className="flex flex-row gap-2 justify-around">
                  <div className="inline-flex flex-row gap-2 pl-3 items-center font-medium justify-center">
                    Quantity
                    <button
                    ...
                    <p className="border border-[#f2f2f2] rounded-sm px-4">
                      {subAdminQuantity}
                      {/* Updated to show Sub-Admin quantity */}
                    </p>
                    <button
                      className="border-2 border-[#f2f2f2] p-1 rounded-sm"
        ...
        <div className="border border-[#F2F2F2] rounded-lg mt-4 p-4">
          ...
          <div className="flex flex-col gap-2 mt-2">
            {storageOptions.map((option) => (
              <div
                key={option.id}
                className="flex flex-col gap-1 p-2 border border-[#F2F2F2] rounded-sm"
              >
                <div className="flex flex-row justify-between items-center">
          ...
          <div className="border border-[#E9EDF6] rounded-sm p-4 bg-[#F6F6F6]">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                {/* Updated to dynamic plan */}
                <span className="text-sm font-bold text-[#1E293B]">
                  ₹{numUsers * getPricePerUser(selectedPlan)}
                </span>{" "}
                {/* Updated to dynamic price */}
              </div>
              <div className="flex justify-between items-center">
                <span className="text-base font-bold text-[#6B7280]">
                  Sub-Admin ({subAdminQuantity}×₹300)
                </span>{" "}
                {/* Updated to dynamic Sub-Admin */}
                <span className="text-sm font-bold text-[#1E293B]">
              ...
              {selectedStorage && (
                <div className="flex justify-between items-center">
                  <span className="text-base font-bold text-[#6B7280]">
                    {
                      storageOptions.find((opt) => opt.id === selectedStorage)
                        .name
                    }
                  </span>{" "}
                  {/* Updated to dynamic storage */}
                  <span className="text-sm font-bold text-[#1E293B]">
                    ₹
                    {
                      storageOptions.find((opt) => opt.id === selectedStorage)
              ...
              {selectedSupport && (
                <div className="flex justify-between items-center">
                  <span className="text-base font-bold text-[#6B7280]">
                    {
                      supportOptions.find((opt) => opt.id === selectedSupport)
                        .name
                    }
                  </span>{" "}
                  {/* Updated to dynamic support */}
                  <span className="text-sm font-bold text-[#1E293B]">
                    ₹
                    {
                      supportOptions.find((opt) => opt.id === selectedSupport)
